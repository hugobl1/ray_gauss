...
class PointCloud:
  ...
  def load_from_pt(self,name_positions="positions",name_spherical_harmonics="spherical_harmonics",name_densities="densities", name_scales="scales", name_quaternions="quaternions",
    ...
    self.positions=torch.load(folder_tensors+'/'+name_positions+'.pt')
    # self.spherical_harmonics=torch.load(folder_tensors+'/'+name_spherical_harmonics+'.pt')#[:,:,:self.harmonic_number]
    color_features=torch.load(folder_tensors+'/'+name_spherical_harmonics+'.pt')
    ...
    self.densities=torch.load(folder_tensors+"/"+name_densities+".pt")
    self.scales=torch.load(folder_tensors+"/"+name_scales+".pt")
    self.quaternions=torch.load(folder_tensors+"/"+name_quaternions+".pt")

    self.sph_gauss_features=torch.load(folder_tensors+"/"+name_sph_gauss_features+".pt")
    self.bandwidth_sharpness=torch.load(folder_tensors+"/"+name_bandwidth_sharpness+".pt")
    self.lobe_axis=torch.load(folder_tensors+"/"+name_lobe_axis+".pt")
  ...
  def restore_model(self,iteration,checkpoint_folder,config_opti=None):
    ...
    data=torch.load(checkpoint_folder + "/chkpnt" + str(iteration) + ".pth")
